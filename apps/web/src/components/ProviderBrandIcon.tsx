import type { ReactNode } from 'react';
import { Blocks } from 'lucide-react';

/**
 * Official brand marks for integration providers, keyed by the provider
 * manifest's `iconKey`. Every mark renders in its authentic color on a neutral
 * chip (the Slack treatment):
 *  - FULL_COLOR: genuine multi-color logos (each brand's own artwork).
 *  - BRAND_GRADIENT: single-path marks filled with the brand's two-tone gradient.
 *  - MONO_KEYS: officially monochrome marks, drawn in `currentColor` so they
 *    adapt to the theme (light on dark, dark on light).
 *  - BRAND_COLOR: officially single-color marks, in their brand color.
 * Single-path silhouettes are from Simple Icons (simpleicons.org, CC0).
 */

const FULL_COLOR: Record<string, { viewBox: string; content: ReactNode }> = {
	slack: {
		viewBox: '0 0 122.8 122.8',
		content: (
			<>
				<path d="M25.8 77.6c0 7.1-5.8 12.9-12.9 12.9S0 84.7 0 77.6s5.8-12.9 12.9-12.9h12.9v12.9z" fill="#E01E5A" />
				<path d="M32.3 77.6c0-7.1 5.8-12.9 12.9-12.9s12.9 5.8 12.9 12.9v32.3c0 7.1-5.8 12.9-12.9 12.9s-12.9-5.8-12.9-12.9V77.6z" fill="#E01E5A" />
				<path d="M45.2 25.8c-7.1 0-12.9-5.8-12.9-12.9S38.1 0 45.2 0s12.9 5.8 12.9 12.9v12.9H45.2z" fill="#36C5F0" />
				<path d="M45.2 32.3c7.1 0 12.9 5.8 12.9 12.9s-5.8 12.9-12.9 12.9H12.9C5.8 58.1 0 52.3 0 45.2s5.8-12.9 12.9-12.9h32.3z" fill="#36C5F0" />
				<path d="M97 45.2c0-7.1 5.8-12.9 12.9-12.9s12.9 5.8 12.9 12.9-5.8 12.9-12.9 12.9H97V45.2z" fill="#2EB67D" />
				<path d="M90.5 45.2c0 7.1-5.8 12.9-12.9 12.9s-12.9-5.8-12.9-12.9V12.9C64.7 5.8 70.5 0 77.6 0s12.9 5.8 12.9 12.9v32.3z" fill="#2EB67D" />
				<path d="M77.6 97c7.1 0 12.9 5.8 12.9 12.9s-5.8 12.9-12.9 12.9-12.9-5.8-12.9-12.9V97h12.9z" fill="#ECB22E" />
				<path d="M77.6 90.5c-7.1 0-12.9-5.8-12.9-12.9s5.8-12.9 12.9-12.9h32.3c7.1 0 12.9 5.8 12.9 12.9s-5.8 12.9-12.9 12.9H77.6z" fill="#ECB22E" />
			</>
		),
	},
	gitlab: {
		viewBox: '0 0 25 24',
		content: (
			<>
				<path
					d="M24.507 9.5l-.034-.09L21.082.827a.896.896 0 00-1.694.091l-2.29 7.01H7.825L5.535.917A.896.896 0 003.84.827L.451 9.41.417 9.5a6.297 6.297 0 002.09 7.278l.01.01.03.022 5.16 3.867 2.556 1.93 1.555 1.176a1.051 1.051 0 001.267 0l1.555-1.175 2.556-1.93 5.184-3.883.014-.01A6.297 6.297 0 0024.507 9.5z"
					fill="#E24329"
				/>
				<path d="M24.507 9.5l-.034-.09a11.44 11.44 0 00-4.56 2.05l-7.447 5.632 4.742 3.584 5.185-3.883.015-.01A6.297 6.297 0 0024.507 9.5z" fill="#FC6D26" />
				<path d="M7.707 20.677l2.556 1.93 1.555 1.176a1.051 1.051 0 001.267 0l1.555-1.175 2.556-1.93-4.742-3.584-5.302 3.583z" fill="#FCA326" />
				<path d="M5.01 11.461a11.43 11.43 0 00-4.56-2.05L.417 9.5a6.297 6.297 0 002.09 7.278l.01.01.03.022 5.16 3.867 4.742-3.584L5.01 11.46z" fill="#FC6D26" />
			</>
		),
	},
	googledrive: {
		viewBox: '0 0 87.3 78',
		content: (
			<>
				<path d="m6.6 66.85 3.85 6.65c.8 1.4 1.95 2.5 3.3 3.3l13.75-23.8h-27.5c0 1.55.4 3.1 1.2 4.5z" fill="#0066da" />
				<path d="m43.65 25-13.75-23.8c-1.35.8-2.5 1.9-3.3 3.3l-25.4 44a9.06 9.06 0 0 0-1.2 4.5h27.5z" fill="#00ac47" />
				<path d="m73.55 76.8c1.35-.8 2.5-1.9 3.3-3.3l1.6-2.75 7.65-13.25c.8-1.4 1.2-2.95 1.2-4.5h-27.502l5.852 11.5z" fill="#ea4335" />
				<path d="m43.65 25 13.75-23.8c-1.35-.8-2.9-1.2-4.5-1.2h-18.5c-1.6 0-3.15.45-4.5 1.2z" fill="#00832d" />
				<path d="m59.8 53h-32.3l-13.75 23.8c1.35.8 2.9 1.2 4.5 1.2h50.8c1.6 0 3.15-.45 4.5-1.2z" fill="#2684fc" />
				<path d="m73.4 26.5-12.7-22c-.8-1.4-1.95-2.5-3.3-3.3l-13.75 23.8 16.15 28h27.45c0-1.55-.4-3.1-1.2-4.5z" fill="#ffba00" />
			</>
		),
	},
	jira: {
		viewBox: '0 0 24 24',
		content: (
			<>
				<defs>
					<linearGradient id="mc-jira-a" x1="0.98" y1="0" x2="0.55" y2="0.44">
						<stop offset=".18" stopColor="#0052CC" />
						<stop offset="1" stopColor="#2684FF" />
					</linearGradient>
					<linearGradient id="mc-jira-b" x1="1" y1="0.09" x2="0.57" y2="0.53">
						<stop offset=".18" stopColor="#0052CC" />
						<stop offset="1" stopColor="#2684FF" />
					</linearGradient>
				</defs>
				<path fill="#2684FF" d="M11.571 11.513H0a5.218 5.218 0 0 0 5.232 5.215h2.13v2.057A5.215 5.215 0 0 0 12.575 24V12.518a1.005 1.005 0 0 0-1.005-1.005z" />
				<path fill="url(#mc-jira-a)" d="M17.294 5.757H5.736a5.215 5.215 0 0 0 5.215 5.214h2.129v2.058a5.218 5.218 0 0 0 5.215 5.214V6.758a1.001 1.001 0 0 0-1.001-1.001z" />
				<path fill="url(#mc-jira-b)" d="M23.013 0H11.455a5.215 5.215 0 0 0 5.215 5.215h2.129v2.057A5.215 5.215 0 0 0 24 12.483V1.005A1.001 1.001 0 0 0 23.013 0z" />
			</>
		),
	},
	confluence: {
		viewBox: '0 0 24 24',
		content: (
			<>
				<defs>
					<linearGradient id="mc-conf-a" x1="0.99" y1="1.12" x2="0.34" y2="0.24">
						<stop offset=".18" stopColor="#0052CC" />
						<stop offset=".92" stopColor="#2684FF" />
					</linearGradient>
					<linearGradient id="mc-conf-b" x1="0.01" y1="-0.12" x2="0.66" y2="0.76">
						<stop offset=".18" stopColor="#0052CC" />
						<stop offset=".92" stopColor="#2684FF" />
					</linearGradient>
				</defs>
				<path fill="url(#mc-conf-a)" d="M.87 18.257c-.248.382-.53.875-.763 1.245a.764.764 0 0 0 .255 1.04l4.965 3.054a.764.764 0 0 0 1.058-.26c.199-.332.454-.763.733-1.221 1.967-3.247 3.945-2.853 7.508-1.146l4.957 2.337a.764.764 0 0 0 1.028-.382l2.364-5.346a.764.764 0 0 0-.382-1c-1.045-.492-3.116-1.469-4.965-2.361C10.911 10.97 5.224 11.185.87 18.257z" />
				<path fill="url(#mc-conf-b)" d="M23.131 5.743c.249-.405.531-.875.764-1.25a.764.764 0 0 0-.256-1.034L18.675.404a.764.764 0 0 0-1.058.26c-.195.335-.451.763-.734 1.225-1.966 3.246-3.945 2.85-7.508 1.146L4.437.694a.764.764 0 0 0-1.027.382L1.046 6.422a.764.764 0 0 0 .382 1c1.039.49 3.105 1.467 4.965 2.361 6.698 3.246 12.392 3.029 16.738-4.04z" />
			</>
		),
	},
	teams: {
		viewBox: '0 0 24 24',
		content: (
			<>
				<path fill="#5059C9" d="M16.5 8.25h5.19c.489 0 .885.396.885.885v4.74a3.27 3.27 0 0 1-3.27 3.27h-.015a3.27 3.27 0 0 1-3.27-3.27V8.775c0-.29.235-.525.48-.525z" />
				<circle fill="#5059C9" cx="19.5" cy="4.875" r="1.875" />
				<circle fill="#7B83EB" cx="12.15" cy="4.35" r="2.85" />
				<path fill="#7B83EB" d="M16.35 8.25H6.42a.93.93 0 0 0-.93.93v6.06a5.7 5.7 0 0 0 5.7 5.7h.15a5.7 5.7 0 0 0 5.7-5.7V9.18a.93.93 0 0 0-.93-.93z" />
				<path fill="#fff" d="M13.32 10.8H8.28v1.29h1.86v5.19h1.32v-5.19h1.86z" />
			</>
		),
	},
};

/** Two-tone marks — the single-path silhouette filled with the brand's gradient [from, to]. */
const BRAND_GRADIENT: Record<string, [string, string]> = {
	bitbucket: ['#2684FF', '#0052CC'],
	onedrive: ['#2A8BF2', '#0364B8'],
	sharepoint: ['#21A0A8', '#036C70'],
	azuredevops: ['#0F8FE0', '#005BA1'],
};

/** Officially monochrome marks — drawn in currentColor so they adapt to the theme. */
const MONO_KEYS = new Set(['github', 'notion', 'zendesk', 'linear']);

/** Officially single-color marks, in their brand color. */
const BRAND_COLOR: Record<string, string> = {
	discord: '#5865F2',
	salesforce: '#00A1E0',
	hubspot: '#FF7A59',
};

const BRAND_ICON_PATHS: Record<string, string> = {
	github:
		'M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12',
	bitbucket:
		'M.778 1.213a.768.768 0 00-.768.892l3.263 19.81c.084.5.515.868 1.022.873H19.95a.772.772 0 00.77-.646l3.27-20.03a.768.768 0 00-.768-.891zM14.52 15.53H9.522L8.17 8.466h7.561z',
	azuredevops:
		'M0 8.877L2.247 5.91l8.405-3.416V.022l7.37 5.393L2.966 8.338v8.225L0 15.707zm24-4.45v14.651l-5.753 4.9-9.303-3.057v3.056l-5.978-7.416 15.057 1.798V5.415z',
	discord:
		'M20.317 4.3698a19.7913 19.7913 0 00-4.8851-1.5152.0741.0741 0 00-.0785.0371c-.211.3753-.4447.8648-.6083 1.2495-1.8447-.2762-3.68-.2762-5.4868 0-.1636-.3933-.4058-.8742-.6177-1.2495a.077.077 0 00-.0785-.037 19.7363 19.7363 0 00-4.8852 1.515.0699.0699 0 00-.0321.0277C.5334 9.0458-.319 13.5799.0992 18.0578a.0824.0824 0 00.0312.0561c2.0528 1.5076 4.0413 2.4228 5.9929 3.0294a.0777.0777 0 00.0842-.0276c.4616-.6304.8731-1.2952 1.226-1.9942a.076.076 0 00-.0416-.1057c-.6528-.2476-1.2743-.5495-1.8722-.8923a.077.077 0 01-.0076-.1277c.1258-.0943.2517-.1923.3718-.2914a.0743.0743 0 01.0776-.0105c3.9278 1.7933 8.18 1.7933 12.0614 0a.0739.0739 0 01.0785.0095c.1202.099.246.1981.3728.2924a.077.077 0 01-.0066.1276 12.2986 12.2986 0 01-1.873.8914.0766.0766 0 00-.0407.1067c.3604.698.7719 1.3628 1.225 1.9932a.076.076 0 00.0842.0286c1.961-.6067 3.9495-1.5219 6.0023-3.0294a.077.077 0 00.0313-.0552c.5004-5.177-.8382-9.6739-3.5485-13.6604a.061.061 0 00-.0312-.0286zM8.02 15.3312c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9555-2.4189 2.157-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.9555 2.4189-2.1569 2.4189zm7.9748 0c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9554-2.4189 2.1569-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.946 2.4189-2.1568 2.4189Z',
	notion:
		'M4.459 4.208c.746.606 1.026.56 2.428.466l13.215-.793c.28 0 .047-.28-.046-.326L17.86 1.968c-.42-.326-.981-.7-2.055-.607L3.01 2.295c-.466.046-.56.28-.374.466zm.793 3.08v13.904c0 .747.373 1.027 1.214.98l14.523-.84c.841-.046.935-.56.935-1.167V6.354c0-.606-.233-.933-.748-.887l-15.177.887c-.56.047-.747.327-.747.933zm14.337.745c.093.42 0 .84-.42.888l-.7.14v10.264c-.608.327-1.168.514-1.635.514-.748 0-.935-.234-1.495-.933l-4.577-7.186v6.952L12.21 19s0 .84-1.168.84l-3.222.186c-.093-.186 0-.653.327-.746l.84-.233V9.854L7.822 9.76c-.094-.42.14-1.026.793-1.073l3.456-.233 4.764 7.279v-6.44l-1.215-.139c-.093-.514.28-.887.747-.933zM1.936 1.035l13.31-.98c1.634-.14 2.055-.047 3.082.7l4.249 2.986c.7.513.934.653.934 1.213v16.378c0 1.026-.373 1.634-1.68 1.726l-15.458.934c-.98.047-1.448-.093-1.962-.747l-3.129-4.06c-.56-.747-.793-1.306-.793-1.96V2.667c0-.839.374-1.54 1.447-1.632z',
	sharepoint:
		'M24 13.5q0 1.242-.475 2.332-.474 1.09-1.289 1.904-.814.815-1.904 1.29-1.09.474-2.332.474-.762 0-1.523-.2-.106.997-.557 1.858-.451.862-1.154 1.494-.704.633-1.606.99-.902.358-1.91.358-1.09 0-2.045-.416-.955-.416-1.664-1.125-.709-.709-1.125-1.664Q6 19.84 6 18.75q0-.188.018-.375.017-.188.04-.375H.997q-.41 0-.703-.293T0 17.004V6.996q0-.41.293-.703T.996 6h3.54q.14-1.277.726-2.373.586-1.096 1.488-1.904Q7.652.914 8.807.457 9.96 0 11.25 0q1.395 0 2.625.533T16.02 1.98q.914.915 1.447 2.145T18 6.75q0 .188-.012.375-.011.188-.035.375 1.242 0 2.344.469 1.101.468 1.928 1.277.826.809 1.3 1.904Q24 12.246 24 13.5z',
	linear:
		'M2.886 4.18A11.982 11.982 0 0 1 11.99 0C18.624 0 24 5.376 24 12.009c0 3.64-1.62 6.903-4.18 9.105L2.887 4.18ZM1.817 5.626l16.556 16.556c-.524.33-1.075.62-1.65.866L.951 7.277c.247-.575.537-1.126.866-1.65ZM.322 9.163l14.515 14.515c-.71.172-1.443.282-2.195.322L0 11.358a12 12 0 0 1 .322-2.195Zm-.17 4.862 9.823 9.824a12.02 12.02 0 0 1-9.824-9.824Z',
	zendesk:
		'M12.914 2.904V16.29L24 2.905H12.914zM0 2.906C0 5.966 2.483 8.45 5.543 8.45s5.542-2.484 5.543-5.544H0zm11.086 4.807L0 21.096h11.086V7.713zm7.37 7.84c-3.063 0-5.542 2.48-5.542 5.543H24c0-3.06-2.48-5.543-5.543-5.543z',
	onedrive:
		'M19.453 9.95q.961.058 1.787.468.826.41 1.442 1.066.615.657.966 1.512.352.856.352 1.816 0 1.008-.387 1.893-.386.885-1.049 1.547-.662.662-1.546 1.049-.885.387-1.893.387H6q-1.242 0-2.332-.475-1.09-.475-1.904-1.29-.815-.814-1.29-1.903Q0 14.93 0 13.688q0-.985.31-1.887.311-.903.862-1.658.55-.756 1.324-1.325.774-.568 1.711-.861.434-.129.85-.187.416-.06.861-.082h.012q.515-.786 1.207-1.413.691-.627 1.5-1.066.808-.44 1.705-.668.896-.229 1.845-.229 1.278 0 2.456.417 1.177.416 2.144 1.16.967.744 1.658 1.78.692 1.038 1.008 2.28z',
	salesforce:
		'M10.006 5.415a4.195 4.195 0 013.045-1.306c1.56 0 2.954.9 3.69 2.205.63-.3 1.35-.45 2.1-.45 2.85 0 5.159 2.34 5.159 5.22s-2.31 5.22-5.176 5.22c-.345 0-.69-.044-1.02-.104a3.75 3.75 0 01-3.3 1.95c-.6 0-1.155-.15-1.65-.375A4.314 4.314 0 018.88 20.4a4.302 4.302 0 01-4.05-2.82c-.27.062-.54.076-.825.076-2.204 0-4.005-1.8-4.005-4.05 0-1.5.811-2.805 2.01-3.51-.255-.57-.39-1.2-.39-1.846 0-2.58 2.1-4.65 4.65-4.65 1.53 0 2.85.705 3.72 1.8',
	hubspot:
		'M18.164 7.93V5.084a2.198 2.198 0 001.267-1.978v-.067A2.2 2.2 0 0017.238.845h-.067a2.2 2.2 0 00-2.193 2.193v.067a2.196 2.196 0 001.252 1.973l.013.006v2.852a6.22 6.22 0 00-2.969 1.31l.012-.01-7.828-6.095A2.497 2.497 0 104.3 4.656l-.012.006 7.697 5.991a6.176 6.176 0 00-1.038 3.446c0 1.343.425 2.588 1.147 3.607l-.013-.02-2.342 2.343a1.968 1.968 0 00-.58-.095h-.002a2.033 2.033 0 102.033 2.033 1.978 1.978 0 00-.1-.595l.005.014 2.317-2.317a6.247 6.247 0 104.782-11.134l-.036-.005zm-.964 9.378a3.206 3.206 0 113.215-3.207v.002a3.206 3.206 0 01-3.207 3.207z',
};

export function ProviderBrandIcon({ iconKey, size = 18, className }: { iconKey: string; size?: number; className?: string }) {
	const full = FULL_COLOR[iconKey];
	if (full) {
		return (
			<svg role="img" viewBox={full.viewBox} width={size} height={size} className={className} aria-hidden="true">
				{full.content}
			</svg>
		);
	}
	const path = BRAND_ICON_PATHS[iconKey];
	if (!path) return <Blocks size={size} className={className} />;

	const gradient = BRAND_GRADIENT[iconKey];
	if (gradient) {
		const id = `mc-grad-${iconKey}`;
		return (
			<svg role="img" viewBox="0 0 24 24" width={size} height={size} className={className} aria-hidden="true">
				<defs>
					<linearGradient id={id} x1="0" y1="0" x2="1" y2="1">
						<stop offset="0" stopColor={gradient[0]} />
						<stop offset="1" stopColor={gradient[1]} />
					</linearGradient>
				</defs>
				<path d={path} fill={`url(#${id})`} />
			</svg>
		);
	}

	const fill = MONO_KEYS.has(iconKey) ? 'currentColor' : (BRAND_COLOR[iconKey] ?? 'currentColor');
	return (
		<svg role="img" viewBox="0 0 24 24" width={size} height={size} fill={fill} className={className} aria-hidden="true">
			<path d={path} />
		</svg>
	);
}
