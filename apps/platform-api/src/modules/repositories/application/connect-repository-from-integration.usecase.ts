...
export class ConnectRepositoryFromIntegrationUseCase {
	...
	async execute(command: ConnectRepositoryFromIntegrationCommand): Promise<{ repository: Repository; jobId: string }> {
		...
		await this.pipelineJobs.create({
			...
			projectId: command.projectId,
			jobType: 'source_sync',
			payload: { connectorId: connector.id, mode: 'full' },
			dedupeKey: `source_sync:${connector.id}:full`,
		});
		await this.sourceSyncQueue.add(
			'ingest',
			{ pipelineJobId, connectorId: connector.id, projectId: command.projectId, mode: 'full' },
			{ jobId: pipelineJobId }
		);
